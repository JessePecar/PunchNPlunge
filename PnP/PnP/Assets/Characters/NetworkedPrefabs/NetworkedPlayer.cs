...
public class NetworkedPlayer : NetworkBehaviour
{
    ...
    public IEnumerator respawnTimer(int respawnTimer){
        ...
        if(isServer){
            this.health = 3;
            GameObject.Find(this.gameObject.name + "Timer").GetComponent<Text>().text = respawnTimer.ToString();
    ...
    public void Cmd_TakeDamage(int amount)
    {
        ...
        if (health <= 0)
        {
            Rpc_updateHearts();
            this.respawnTimer(5);
            /* Change this to a respawn Timer */            
            NetworkServer.Destroy(this.playerObject);
        }
        Rpc_updateHearts();
    }
